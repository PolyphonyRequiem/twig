...
- EPIC-003: Filter Mode & Link Traversal **[DONE]**

| Task | Description | Status | Relevant Files |
|------|-------------|--------|----------------|
| ITEM-012 | Implement filter mode in `RenderInteractiveTreeAsync` keyboard dispatch. When `IsFilterMode` is false and user presses an alphanumeric key: set `IsFilterMode = true`, append character to `FilterText`, call `state.ApplyFilter(filterText)` which filters `VisibleSiblings` to items whose Title contains the filter text (case-insensitive). When `IsFilterMode` is true: Backspace removes last character (if empty, exit filter mode), Escape clears filter and exits filter mode, Enter selects the filtered item at cursor position and exits filter mode, alphanumeric appends to filter. Filter visual: show `Filter: {text}_` bar below the tree in the renderable. | Done | `src/Twig/Rendering/SpectreRenderer.cs` |
| ITEM-013 | Implement link traversal in `RenderInteractiveTreeAsync` keyboard dispatch. Tab key: (1) get combined links via `state.GetCombinedLinks()`, (2) if no links, ignore, (3) increment `LinkJumpIndex` (wrapping to 0), (4) get the target ID from the link at current index, (5) call `loadNodeState(targetId)` to load the linked item's full tree context, (6) replace the current state with the new state. Shift+Tab cycles backward. The preview panel already shows the link list — highlight the current link jump target in the list with `[aqua]` color. | Done | `src/Twig/Rendering/SpectreRenderer.cs` |
| ITEM-014 | Handle missing linked items: if `loadNodeState(targetId)` returns a state where `CursorItem` is null (item not in cache), show `[yellow]Item #{targetId} not in cache. Run 'twig refresh' to fetch.[/]` in the preview panel instead of the item details. Do not change the tree view — keep the current tree and show the error in the preview only. Advance to the next link on the next Tab press. | Done | `src/Twig/Rendering/SpectreRenderer.cs` |
| ITEM-015 | Add unit tests for filter mode: verify filter narrows VisibleSiblings, cursor resets to 0 on filter change, empty filter restores all siblings, Escape clears filter. Add unit tests for link traversal: verify Tab cycles through links in order, Shift+Tab cycles backward, missing items show error message, empty link list ignores Tab. | Done | `tests/Twig.Cli.Tests/Rendering/TreeNavigatorStateTests.cs` |

## 14. Change Log

- 2026-03-27: EPIC-003 review fixes round 2 — Removed dead `predecessorPos` variable from `BuildPreviewPanel_LinkHighlight_HighlightsCorrectLink` test (leftover from prior fix that switched the assertion to use `relatedPos`). Added clarifying comment. All 3,246 tests pass.

- 2026-03-27: EPIC-003 review fixes — (1) Fixed `BuildPreviewPanel_LinkHighlight_HighlightsCorrectLink` negative assertion: changed `pos > predecessorPos` to `pos > relatedPos` so the predicate correctly rejects aqua codes bleeding past the highlighted link (TEST-NEGATIVE-ASSERTION), (2) Updated stale XML doc on `NavigatorAction.FilterCleared` to reflect that the render loop now performs async work for this action (STALE-XMLDOC). All 3,246 tests pass.

- 2026-03-27: EPIC-003 DONE — Implemented filter mode (ITEM-012): alphanumeric keys enter filter mode, Backspace removes characters (exits on empty), Escape clears filter, Enter accepts filter and returns to navigate mode. Implemented link traversal (ITEM-013): Tab/Shift+Tab cycle through combined links via `AdvanceLinkJump()`/`ReverseLinkJump()`, preview panel highlights active link with `[aqua]`. Missing linked items (ITEM-014): show `[yellow]` error in preview panel, keep current tree. Added `AcceptFilter()`, `AdvanceLinkJump()`, `ReverseLinkJump()` to `TreeNavigatorState`; `LinkJumpIndex` initializes at -1. Added `FilterUpdated`/`LinkJump` to `NavigatorAction` enum. 27 new tests (13 state + 14 render). All 3,245 tests pass.
