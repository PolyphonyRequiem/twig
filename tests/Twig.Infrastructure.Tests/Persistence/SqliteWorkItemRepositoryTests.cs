...
public class SqliteWorkItemRepositoryTests : IDisposable
{
    ...
    // ═══════════════════════════════════════════════════════════════
    //  ClearPhantomDirtyFlagsAsync tests (#1335 / #1395)
    // ═══════════════════════════════════════════════════════════════

    [Fact]
    public async Task ClearPhantomDirtyFlagsAsync_ReturnsZero_WhenNoDirtyItems()
    {
    ...
    [Fact]
    public async Task ClearPhantomDirtyFlagsAsync_ClearsDirtyItemWithNoPendingChanges()
    {
        var item = CreateWorkItem(1, "Task", "Phantom Dirty", "Active");
        item.SetDirty();
        await _repo.SaveAsync(item);
    ...
    [Fact]
    public async Task ClearPhantomDirtyFlagsAsync_PreservesDirtyItemWithPendingChanges()
    {
        var item = CreateWorkItem(1, "Task", "Real Dirty", "Active");
        item.SetDirty();
        await _repo.SaveAsync(item);

        // Insert a pending_changes row directly
        var conn = _store.GetConnection();
        using var cmd = conn.CreateCommand();
        cmd.CommandText = """
            INSERT INTO pending_changes (work_item_id, change_type, field_name, old_value, new_value, created_at)
            VALUES (1, 'FieldUpdate', 'System.Title', 'Old', 'New', '2026-01-01T00:00:00Z');
            """;
        cmd.ExecuteNonQuery();
    ...
    [Fact]
    public async Task ClearPhantomDirtyFlagsAsync_SkipsSeedItems()
    {
    ...
    [Fact]
    public async Task ClearPhantomDirtyFlagsAsync_MixedScenario_ClearsOnlyPhantoms()
    {
        // Item 1: dirty, no pending changes → should be cleared (phantom)
        var phantom = CreateWorkItem(1, "Task", "Phantom", "Active");
        phantom.SetDirty();
        await _repo.SaveAsync(phantom);

        // Item 2: dirty, has pending changes → should stay dirty
        ...
        cmd.CommandText = """
            INSERT INTO pending_changes (work_item_id, change_type, field_name, old_value, new_value, created_at)
            VALUES (2, 'FieldUpdate', 'System.Title', 'Old', 'New', '2026-01-01T00:00:00Z');
            """;
        cmd.ExecuteNonQuery();

        // Item 3: clean → should stay clean
        var clean = CreateWorkItem(3, "Task", "Clean", "Active");
        await _repo.SaveAsync(clean);

        // Item 4: seed, dirty, no pending changes → should stay dirty (seed excluded)
        WorkItem.InitializeSeedCounter(-100);
        var seed = WorkItem.CreateSeed(WorkItemType.Task, "Dirty Seed");
        seed.SetDirty();
        await _repo.SaveAsync(seed);
    ...
    [Fact]
    public async Task ClearPhantomDirtyFlagsAsync_ReturnsZero_WhenEmptyDatabase()
    {
        var cleared = await _repo.ClearPhantomDirtyFlagsAsync();
        cleared.ShouldBe(0);
    }
