...
/// <summary>
/// Opaque holder descriptor forwarded to the ADO projection. Carries two
/// values captured at mint time:
/// <list type="bullet">
///   <item><see cref="Identity"/>: the stable identity string persisted in
///     the claim record. AB#739's readback verification requires the
///     resolver to supply the stable ADO identity (UPN / uniqueName /
///     descriptor) here, not a display rendering — the projection writes
///     this string as <c>System.AssignedTo</c> and byte-compares the
///     observed stable identity from ADO's readback against it.</item>
