...
8. **Decompose large issues into task chains** — If an issue is estimated to take more than 30 minutes of implementation effort, decompose it into a sequence of Tasks before coding. Use `twig seed chain` to create them as successor-linked seeds under the parent Issue, then publish and execute them one at a time. Each task should be independently committable and testable. Fully close each task (commit, tests pass, Done transition applied through the change-proposal path and confirmed by `twig show <id> --refresh`) before starting the next. `twig state Done` is **not** the close mechanism — it writes `System.State` alone and cannot carry the close-gate fields the process requires; stage them in the same `batch` op as the transition and apply with `twig proposal apply --confirm <digest> --authorize <identity>`.
